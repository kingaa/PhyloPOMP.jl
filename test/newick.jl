...
@testset verbose=true "Newick parser" begin
    ...
    g=parse_newick("():0.1;",Val(SEIR),0.0);
    @test length(g.nodes)==2
    @test g.nodes[2].type==PhyloPOMP.Sample
    g = parse_newick("A():3.2;",Val(SEIR),0.0);
    @test g.nodes[2].type==PhyloPOMP.Sample
    g = parse_newick("A([&&PhyloPOMP type=node]):3.2;",Val(SEIR),0.0);
    @test g.nodes[2].type==PhyloPOMP.Sample
    @test_warn "dropping zero-length branch" g = parse_newick("A()[&&PhyloPOMP type=sample]:3.2;",Val(SEIR),0.0);
    @test g.nodes[2].type==PhyloPOMP.Sample
    g = parse_newick("A:4;()B:3;",Val(SEIR),0.0,5.0);
    @test length(g.nodes)==4

    @test_throws "unbalanced parentheses" parse_newick("(:0.1;",Val(SEIR),0.0)
    @test_throws "unbalanced square brackets" parse_newick(")3:1;(((:0.1)),[&&PhyloPOMP:deme=E]type=sample]:1.00,(((:0.3,:0.1),),):0.3)a:0.5;",Val(SEIR),0.0)
    @test_throws "unbalanced square brackets" parse_newick("yloPOMP:deme=E|type=node]:1.00,(((:0.3,:0.1),),):0.3)a:0.5;",Val(SEIR),0.0)
    @test_throws "unbalanced parentheses" parse_newick(")3:1;(((:0.1)),[&&PhyloPOMP:deme=E|type=sample]:1.00,(((:0.3,:0.1),),):0.3)a:0.5;",Val(SEIR),0.0)
    @test_throws "unbalanced parentheses" parse_newick(")3:1;(((:0.1)),[&&PhyloPOMP:deme=E|type=sample]:1.00,(((:0.3,:0.1),),):0.3)a:0.5;",Val(SEIR),0.0)
    @test_throws "misplaced comma or" parse_newick("((:0.1)),([&&PhyloPOMP:deme=E|type=sample]:1.00,(((:0.3,:0.1),),):0.3)a:0.5;",Val(SEIR),0.0)
    @test_throws "unrecognized deme" parse_newick("([&&PhyloPOMP:deme=tim|type=bob]:1.0000):0.5;",Val(SEIR),0.0)
    @test_throws "unrecognized type" parse_newick("([&&PhyloPOMP:deme=E|type=bob]:1.0000):0.5;",Val(SEIR),0.0)
    @test_throws "unrecognized type" parse_newick("(()[&&PhyloPOMP:deme=I|type=extant]:1.0000):0.5;",Val(SEIR),0.0)
    @test_throws "no final semicolon" parse_newick("([&&PhyloPOMP|deme=9|type=sample]:1.0000):0.5",Val(SEIR),0.0)
    @test_throws r"cannot parse .* as Float64" parse_newick("([&&PhyloPOMP|deme=E|type=sample]:1.0000A):0.5;",Val(SEIR),0.0)
    @test_throws "invalid base 10 digit" parse_newick("([&&PhyloPOMP|deme=E|type=sample]:1.0000A):0.5;",Val(SEIR),0)
    @test_throws "unbalanced parentheses" parse_newick("(:1,:1):1;((:1,:1):1;",Val(SEIR),0.0)
    @test_throws "missing comma or semicolon" parse_newick("()();",Val(SEIR),0.0)
    @test_throws "unbalanced parentheses" parse_newick("(A();",Val(SEIR),0.0)
    @test_throws "misplaced comma or unbalanced parentheses" parse_newick(",();",Val(SEIR),0.0)
    @test_throws "misplaced comma or unbalanced parentheses" parse_newick(":2 ,:3 ();",Val(SEIR),0.0)
    @test_throws "unbalanced square brackets" parse_newick("()[bob =3 jack=[4]:0.3;",Val(SEIR),0.0)
