...
@testset verbose=true "finite-state Markov" begin
    ...
    @test isa(transition(p,0),Matrix{Float32})

    p = fsmarkov(I1=>1,I3=>2,(I1,I2)=>3,(I2,I3)=>1,I2=>1);
    @test sum(statdist(p))==1
    @test sum(abs.(generator(p)))==4.5
    @test all(sum(generator(p),dims=1).==0)
    @test_throws r"size mismatch" transition(p,10,[1,2])
    @test maximum(abs.(transition(p,0,[1,2,3])-[1,2,3]))+100-100==0
    @test maximum(abs.(transition(p,1e5,[1,1,1])-[0.75,0.75,1.5]))+100-100==0
