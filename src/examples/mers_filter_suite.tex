...
\begin{document}
\begin{center}
{\large\bfseries MERS Filtering Equations (v29)}\\[2pt]
...
\textbf{This does not match the currently-checked-out \texttt{mers.yml}} (the two most recently
modified copies found, dated after v25's citation, both still specify the constant/guarded
rate for \emph{both} demes) --- i.e.\ the Julia \texttt{devel} branch and the R reference model
currently disagree on this term, and this document does not resolve which is intended; it
records the Julia-side choice actually shipped in this repository's kernels. See \S8.2 below
...
from v25). \textbf{This is not currently matched by the checked-out \texttt{mers.yml}}, whose
\texttt{death\_c}/\texttt{death\_h} marks both still use the constant, guarded form
(\texttt{rate params.Bh}; \texttt{jump if(Sh>0) Sh-=1}) that v25 verified against; see the v28
changelog note. The sampling marks SC, SH are singular and contribute nothing on open intervals; their
