...
\begin{document}
\begin{center}
{\large\bfseries MERS Filtering Equations (v30)}\\[2pt]
...
guided-family kernels: \texttt{knowledge!} and \texttt{singular\_part!} (the root- and
fork-coloring proposals of the last paragraph of \S\ref{sec:proposal-kernel}, and the shared
\texttt{filter\_pomp} constructor with its \texttt{proposal\_floor} parameter, i.e.\
$\varepsilon$), together with the epsilon-floor building blocks used by the three kernels below:
\texttt{floored\_shares} and \texttt{floored\_shares\_feasible} implement $\pi_\varepsilon(j)=
(1-\varepsilon)\pi_0(j)+\varepsilon/n$ over a feasible outcome set of size $n$;
\texttt{cross\_proposal} implements the na\"{i}ve/Soft identity-vs-tracked-group split
$(\pi_0(0),\sum_{j\ge1}\pi_0(j))$; \texttt{floored\_branch\_law} implements Soft's per-branch
$\pi_\varepsilon(b)$; \texttt{hard\_branch\_law} implements Hard's $\kappa_\varepsilon(b)$.

\paragraph{Relation to this repository's SEIR implementation.} The three guided-family kernels
were designed by generalizing this repository's existing single-deme-swap SEIR implementation
(\texttt{seir\_soft.jl}, \texttt{seir\_guided.jl}, \texttt{seir\_hard.jl}, and their shared
\texttt{seir\_funs.jl}) to MERS's two-host, two-direction cross-deme structure, and by adding the
$\varepsilon$-floor, which the SEIR implementation does not have at all (every SEIR kernel is
support-\emph{unsafe} in the same way the na\"{i}ve MERS kernel was before this floor was added).
Concretely:
\begin{itemize}
\item \textbf{Soft} (\texttt{mers\_soft.jl}) mirrors \texttt{seir\_soft.jl}'s architecture: an
aggregate identity/tracked-group split (there, via \texttt{seir\_funs.jl}'s generic
\texttt{transmission!}, parameterized by \texttt{onI=ellI}) followed by a conditional branch draw
via \texttt{PhyloPOMP.choose\_branch(rh, n, cols, i, j)} (the low-level, precomputed-hazard
overload in \texttt{guide.jl}, populated once per interval by \texttt{relhaz\_alloc}/
\texttt{relhaz!}). MERS's version keeps the same two-stage shape but replaces the split and the
conditional draw with the floored \texttt{cross\_proposal}/\texttt{floored\_branch\_law} above, and
generalizes from one swap direction to two (camel$\to$human and human$\to$camel).
...
overload in \texttt{guide.jl} that recomputes the hazard on demand). Because that function has no
floor, and \texttt{guide.jl} is shared code that \texttt{seir\_guided.jl}/\texttt{seir\_hard.jl}
depend on unfloored, \texttt{mers\_guided.jl} does not edit it; instead it defines a local,
floored analogue, \texttt{floored\_choose\_branch}, built from the shared \texttt{floored\_shares}
helper.
\item \textbf{Hard} (\texttt{mers\_hard.jl}) mirrors \texttt{seir\_hard.jl}'s architecture: the
same \texttt{seir\_funs.jl} generic \texttt{transmission!}, but parameterized by
\texttt{onI=sum\_relhaz(...)} (the raw hazard sum, not the tracked-lineage count), and the same
\texttt{choose\_branch(rh, n, cols, i, j)} for the conditional branch draw. MERS's version replaces
the unnormalized rate split with the floored \texttt{cross\_proposal}/\texttt{hard\_branch\_law}
above, and keeps the compensating-decay term (\texttt{decay} accumulating
\texttt{rate\ -\ alpha[identity]\ -\ alpha[swap-group]}) that \texttt{seir\_hard.jl} already uses
for the same purpose.
\end{itemize}
One structural difference from the SEIR baseline: \texttt{seir\_guided.jl} does not
\texttt{include("seir\_funs.jl")} (it duplicates \texttt{singular\_part!} instead), whereas all
three MERS kernels \texttt{include("mers\_funs.jl")}, since the epsilon-floored root/fork proposal
is shared identically across all three here.
...
zero-infected and constructor checks). Beyond that:
\begin{itemize}
\item \texttt{test/mers\_soft.jl} additionally checks $\ell=0\Rightarrow\pi_\varepsilon(0)=1$;
strict positivity of every outcome at the $I=\ell$ boundary; that
$\sum_{b\ge1}\pi_\varepsilon(b)$ from \texttt{floored\_branch\_law} agrees with
\texttt{cross\_proposal}'s tracked-group mass (the aggregate-plus-conditional equivalence,
numerically); and that Soft's and Guided's log-likelihood estimates on the same genealogy and a
deliberately nonuniform guide differ by more than Monte Carlo noise (the Soft-vs-Guided
distinction argued in \S\ref{sec:proposal-kernel}).
\item \texttt{test/mers\_hard.jl} additionally checks the same boundary/positivity properties for
\texttt{hard\_branch\_law}, the aggregate-plus-conditional equivalence for $\kappa_\varepsilon$,
and the rate/weight identity $\beta_j=\alpha\,\kappa_\varepsilon(j)$ together with
$\log(\alpha\Phi_j/\beta_j)=\log(\Phi_j/\kappa_\varepsilon(j))$ from \S\ref{sec:proposal-kernel}.
\item \texttt{test/mers\_guided.jl} checks the joint-normalization boundary/positivity properties
directly on \texttt{floored\_shares} (the building block of \texttt{floored\_choose\_branch}), and
that a dominant relative hazard pushes the joint identity share below the fixed na\"{i}ve ratio
$(I-\ell)/I$, as \S\ref{sec:proposal-kernel} argues.
\end{itemize}
